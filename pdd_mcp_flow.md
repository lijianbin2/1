# 拼多多 MCP 全流程 - 存档 v1.34 (2026-09-04 01:28)
...
> v1.34 点亮前加回 立即领取（01:28 用户指令：任意 立即领取 点后变 去使用，已手动点），去领取(去领取)已移除保留 立即领取。
...
| s7 立即领取 | 任意一个 立即领取（点后变 去使用，已手动点击验证 01:28） | 227,1048 640,1048 940,1048 备选 dump解析 | s7 点后变 去使用 |
| s8 点亮/滑动 | 滑动10秒(可选) | 立即点亮 830,410 -> 去看看 485,1485 -> swipe 640,1700->900 x7 -> 粉条消失 -> 顶部 910,855 或 swipe 600,700->1400*2 | all done |
| 结束 | 点亮后结束 | s8滑动完成后结束（s7 立即领取→去使用 已前置） | === all done v1.34 === |

## 核心坐标
省钱月卡 895,766（回顶部后）  百亿 110,1360  会员 1120,233  打卡 640,1280
立即领取 227,1048/640,1048/940,1048（点后变 去使用） 消费券红块640,680
...
日志标志：pdd v1.34, s3 1120,233, s4 640,1280, s6 640,680, s7 立即领取->去使用, s8 点亮, all done
...
- 01:27 s7 单步验证：dump2 去领取 [903,503][1092,578] cx997,540 已验证（v1.32），01:28 按用户指令移除该步骤（v1.33）
