...
function __convertArgs() {
  ...
  return Object.assign({ grouptype: 1 }, userArgs);
}
...
// 其次使用 Sub-Store 注入的 $substore.http
async function __fetchText(url) {
  ...
  if (typeof $substore !== "undefined" && $substore && $substore.http && typeof $substore.http.get === "function") {
    const resp = await $substore.http.get({ url: url, timeout: 15000 });
...
// 获取中间脚本入口：本地缓存 -> 远程最新 -> 内联快照
async function __loadConvertMain(args) {
  const cache = globalThis.__CONVERT_CACHE__ || (globalThis.__CONVERT_CACHE__ = {});
  const now = Date.now();

  if (cache.main && now - cache.time < CONVERT_CACHE_TTL) {
    return cache.main;
  }
...
// 快照日期见 build 时的控制台输出，可用 build-substore-combined.js 刷新
// ============================================================================
