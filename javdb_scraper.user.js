...
// @version      5.7.1
// @description  【极速冲刺版】移除休眠机制，请求间隔压缩至 0.3s；遇封禁 3s 自动切数字域名复活；精准精选字幕版与迅雷 txt 导出。改进：锁超时回收与相对 URL 解析。
// @author       Assistant
// @license      MIT
// @match        *://*.javdb574.com/*
// @match        *://javdb574.com/*
...
(function () {
  ...
  panel.innerHTML = `
    <div id="scraper-header" style="font-weight: bold; margin-bottom: 8px; font-size: 14px; border-bottom: 1px solid #444; padding-bottom: 4px; cursor: move; user-select: none; display: flex; justify-content: space-between; align-items: center;">
      <span>⚡ JavDB 磁链提取器 v5.7.1 (极速版)</span>
    ...
    try {
      ...
      if (genreTarget) {
        ...
        tagElements.forEach(el => {
          if (el.textContent.trim().toLowerCase() === genreTarget.toLowerCase() || el.textContent.trim().includes(genreTarget)) {
            matched = true;
          }
        });
        ...
        if (!matched) {
          ...
          if (!infoPanel.textContent.includes(genreTarget)) {
  ...
  async function runScraper() {
    ...
    try {
      if (currentMode === 'current') {
        ...
        const pageTitle = (document.title.split('-')[0] || 'JavDB_列表').trim().replace(/[\\/:*?"<>|]/g, '');
        ...
        for (let i = startNum; i <= endNum; i++) {
          ...
          for (const term of searchTerms) {
            ...
            try {
              updateLockHeartbeat();
              const searchRes = await fetch(`/search?q=${encodeURIComponent(term)}&f=all`);
              if (searchRes.status === 429) {
                log(`⚠️ 触发限流，休眠 10 秒...`);
                await sleep(10000);
                continue;
        ...
        for (let pIdx = 0; pIdx < pagesToVisit.length; pIdx++) {
          ...
          try {
            updateLockHeartbeat();
            const searchRes = await fetch(`/search?q=${encodeURIComponent(actorName)}&page=${page}&f=all`);
            if (searchRes.status === 429) {
              log(`⚠️ 触发限流，休眠 10 秒...`);
              await sleep(10000);
              pIdx--;
  ...
  function downloadTXT(magnets, fileNameTag) {
    const valid = magnets.filter(m => m && m.startsWith('magnet:?'));
