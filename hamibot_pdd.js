...
function pureShellBack(){ try{ back(); }catch(e){} sleep(300); try{ shell("input keyevent 4"); }catch(e){} sleep(600); try{ back(); }catch(e){} sleep(300); }
...
function swipeToTop(){ try{ for(let i=0;i<2;i++){ swipe(600,700,600,1400,600); sleep(900);} sleep(300); }catch(e){ console.log("swipeToTop err "+e); try{ shell("input swipe 600 700 600 1400 600");}catch(e2){} sleep(800); } }
...
function clickViaParent(selectorText){ try{
    ...
    for(let i=0;i<5;i++){
        if(cur.clickable()){ console.log("点击可点击节点层级"+i+" "+cur.bounds()); try{cur.click();}catch(e){} sleep(200);
            try{ let b=cur.bounds(); shell("input tap "+b.centerX()+" "+b.centerY());}catch(e){}
            return true; 
        }
    ...
    try{ shell("input tap "+b.centerX()+" "+b.centerY()); }catch(e){ console.log("shell tap失败 "+e); }
    try{ let pb=cur.bounds(); shell("input tap "+pb.centerX()+" "+pb.centerY());}catch(e){}
    return true;
        ...
        for(let i=0;i<cands.length;i++){
            ...
            for(let j=0;j<5;j++){
                if(cur.clickable()){
                    ...
                    sleep(200);
                    try{ let bb=cur.bounds(); shell("input tap "+bb.centerX()+" "+bb.centerY()); }catch(e){}
                    return true;
                }
                let p=cur.parent(); if(!p) break; cur=p;
            }
            ...
            sleep(200);
            try{ click(b.centerX(), b.centerY()); }catch(e){}
            try{ shell("input tap "+b.centerX()+" "+b.centerY()); }catch(e){}
            try{ let pb=cur.bounds(); shell("input tap "+pb.centerX()+" "+pb.centerY()); }catch(e){}
            return true;
        }
    }
...
function ensurePdd(){
    ...
    if(pkg!=="com.xunmeng.pinduoduo"){
        ...
        try{ shell("am start -n com.xunmeng.pinduoduo/com.xunmeng.pinduoduo.ui.activity.MainFrameActivity");}catch(e){
        ...
        if(pkg!=="com.xunmeng.pinduoduo"){
            ...
            try{ shell("monkey -p com.xunmeng.pinduoduo -c android.intent.category.LAUNCHER 1");}catch(e){}
            sleep(4000);
            pkg=currentPackage();
    ...
    try{ shell("svc power stayon true"); }catch(e){}
    sleep(1200);
}
function goHome(){
    console.log("goHome 开始 isHome="+isHome()+" pkg="+currentPackage()+" isCommodity="+isCommodityPage());
    try{ shell("svc power stayon true"); shell("input keyevent 224"); }catch(e){}
    hideConsoleSoon();
    for(let i=0;i<10;i++){
        ...
        if(tab){ console.log("点 首页tab "+tab.bounds()); try{tab.click();}catch(e){} 
            try{ let b=tab.bounds(); shell("input tap "+b.centerX()+" "+b.centerY());}catch(e){}
        ...
        console.log("back "+(i+1)+"/10 robustBack");
    ...
    try{ shell("input tap "+Math.round(w*0.12)+" "+Math.round(h*0.93));}catch(e){}
    sleep(1800);
...
function s1(){
    ...
    for(let pt of ptsDirect){
        ...
        tapShell(pt[0],pt[1]);
        sleep(600);
        try{ shell("input tap "+pt[0]+" "+pt[1]);}catch(e){}
        sleep(1800);
        if(!isHome()){
            if(isCommodityPage()){
            ...
            // v1.21: avoid over-back flip-flop, check via isHome safe + fallback to pureShellBack once
            try{
                if(!isHome()){
    ...
    for(let k=0;k<4;k++){
        ...
        if(n){
            ...
            let cur=n;
            for(let i=0;i<5;i++){
                if(cur.clickable()){ try{cur.click();}catch(e){} try{ let b=cur.bounds(); shell("input tap "+b.centerX()+" "+b.centerY());}catch(e){} ok=true; break; }
                let p=cur.parent(); if(!p) break; cur=p;
            }
            if(!ok){ try{n.click();}catch(e){} try{ let b=n.bounds(); shell("input tap "+b.centerX()+" "+b.centerY());}catch(e){} ok=true; }
            sleep(2800);
            if(!isHome()) break;
        }
        if(ok && !isHome()) break;
    ...
    if(isHome()){
        ...
        if(isHome()){
            try{ let n=text("省钱月卡").findOne(500)||descContains("省钱月卡").findOne(500); if(n){ let bb=n.bounds(); console.log("最后bounds中心 "+bb.centerX()+","+bb.centerY()); shell("input tap "+bb.centerX()+" "+bb.centerY()); sleep(2200); } }catch(e){}
        }
...
function s2(){
    ...
    for(let k=0;k<6;k++){
        ...
        if(n){ console.log("百亿模糊 "+n.text()+" "+n.bounds()); try{n.click();}catch(e){} try{shell("input tap "+n.bounds().centerX()+" "+n.bounds().centerY());}catch(e){} ok=true; sleep(3000); break; }
...
function s4(){
    ...
    if(d){
        ...
        try{ click(b.centerX(), b.centerY()); }catch(e){}
        sleep(400);
        try{ shell("input tap 385 1180");}catch(e){}
        sleep(3000);
...
function s6(){
    ...
    for(let k=0;k<5;k++){
        ...
        if(text("百亿消费券").exists()){
            ...
            if(n && n.bounds().centerY()<=2500 && n.text().indexOf("开学")<0){
                ...
                for(let i=0;i<5;i++){ if(cur.clickable()){ try{cur.click();}catch(e){} clicked=true; try{ let b=cur.bounds(); shell("input tap "+b.centerX()+" "+b.centerY());}catch(e){} break; } let pp=cur.parent(); if(!pp) break; cur=pp; }
                if(!clicked) try{n.click();}catch(e){}
                try{ let b=n.bounds(); shell("input tap "+b.centerX()+" "+b.centerY()); }catch(e){}
                sleep(3200);
    ...
    if(n2 && n2.text().indexOf("开学")<0 && n2.bounds().centerY()<=2500){
        ...
        try{ let b=n2.bounds(); shell("input tap "+b.centerX()+" "+b.centerY()); }catch(e){}
        sleep(3200);
...
function s7(){
    ...
    if(els&&els.length>0){ 
        ...
        for(let i=0;i<Math.min(3, els.length); i++){
            ...
            try{ let b=e.bounds(); shell("input tap "+b.centerX()+" "+b.centerY());}catch(err){}
            sleep(1200);
        }
    } else { 
...
function s8(){
    ...
    if(light){
        ...
        let cur=light; for(let i=0;i<4;i++){ if(cur.clickable()){try{cur.click();}catch(e){} try{shell("input tap "+cur.bounds().centerX()+" "+cur.bounds().centerY());}catch(e){} break;} cur=cur.parent(); if(!cur) break; }
        ...
        if(go){ 
            ...
            let c=go; for(let i=0;i<3;i++){ if(c.clickable()){try{c.click();}catch(e){} try{shell("input tap "+c.bounds().centerX()+" "+c.bounds().centerY());}catch(e){} break;} c=c.parent(); if(!c) break; }
...
function main(){
    ...
    if(!inPdd){
        ...
        sleep(1500);
        inPdd=isInPdd();
        console.log("重试1 inPdd="+inPdd);
        if(!inPdd){ sleep(1500); inPdd=isInPdd(); console.log("重试2 inPdd="+inPdd); }
        if(!inPdd){ console.log("仍不在pdd，尝试拉起一次"); try{ shell("monkey -p com.xunmeng.pinduoduo -c android.intent.category.LAUNCHER 1");}catch(e){} sleep(3000); inPdd=isInPdd(); console.log("拉起后 inPdd="+inPdd); }
