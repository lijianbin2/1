...
function s1(){
    ...
    if(isHome() || textContains("省钱月卡").exists() || descContains("省钱月卡").exists()){
        ...
        let w=device.width,h=device.height;
        let pts=[]; pts.push([640,427]); pts.push([Math.round(w*0.50), Math.round(h*0.155)]); pts.push([Math.round(w*0.70), Math.round(h*0.285)]);
        pts.push([896,784]);
        pts.push([895,766]);
        pts.push([896,700]);
        pts.push([896,860]);
...
function s3(){
    ...
    if(!ok){ console.log("会员文本未找到，坐标兜底"); try{ click(1130,165);}catch(e){} try{ shell("input tap 1130 165", true);}catch(e){} }
    sleep(2800);
    showConsoleSoon();
}
function s4(){
    ...
    if(!ok){ console.log("打卡未找到，坐标兜底"); try{ click(385,1180);}catch(e){} try{ shell("input tap 385 1180", true);}catch(e){} }
    sleep(2200);
    showConsoleSoon();
}
...
function s6(){
    ...
    let pts=[[900,700],[640,770],[640,650],[640,850],[896,784],[700,900],[500,800]];
    for(let pt of pts){
        console.log("尝试坐标 "+pt[0]+","+pt[1]);
        try{ click(pt[0],pt[1]); }catch(e){}
        try{ shell("input tap "+pt[0]+" "+pt[1], true); }catch(e){}
        sleep(1000);
        if(textContains("共200元券").exists() || text("立即领取").exists() || (textContains("消费券").exists() && !isHome())){
            console.log("坐标 "+pt+" 似乎已进入");
            break;
        }
...
function s7(){
    console.log("=== s7 立即领取 (WebView坐标兜底) ===");
    hideConsoleSoon();
    ...
    if(els&&els.length>0){ 
        ...
        for(let pt of pts){
            ...
            try{ shell("input tap "+pt[0]+" "+pt[1], true);}catch(e){}
            sleep(1000);
            if(text("立即点亮").exists()||desc("立即点亮").exists()||textContains("还剩").exists()){
                console.log("检测到点亮/还剩，已触发领取");
                break;
            }
        }
    }
...
function s8(){
    ...
    if(light){
        ...
        let cur=light;
        for(let i=0;i<4;i++){ if(cur.clickable()){try{cur.click();}catch(e){} try{shell("input tap "+cur.bounds().centerX()+" "+cur.bounds().centerY(), true);}catch(e){} break;} cur=cur.parent(); if(!cur) break; }
        ...
        } else { console.log("未找到去看看，坐标 640,1360"); try{click(640,1360);}catch(e){} try{shell("input tap 640 1360", true);}catch(e){} }
        sleep(2800);
        swipeEdgeUp(5,2000);
