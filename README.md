...
每次接一个项目时，流程只有下面这条主线。不要把网盘交付和闲鱼发布混成一个表单：
...
| 阶段 | 输入 | 产出 |
| --- | --- | --- |
| 素材核验 | 项目源目录、项目名称 | 文件数量、体积、代表图片清单 |
| 图片生成 | 源目录、输出目录 | `01.png` 至 `04.png` |
| 文案生成 | 项目事实、适合人群、交付规则 | 闲鱼正文文件 |
| 分享制作 | 已创建的项目文件夹 | 标题、私密永久链接、提取码 |
| 浏览器填写 | 四张图片、正文、价格、分类 | 已上传但未发布的闲鱼表单 |
| 发布复核 | 当前表单和正文检查结果 | 用户可确认的发布摘要 |
| 发布交付 | 用户明确确认 | 闲鱼商品结果 + 独立网盘分享信息 |

### 状态流转

```text
待核验 → 已核验 → 已生成图片 → 已生成文案
       → 已生成分享块 → 已上传 → 待确认发布 → 已发布
```

任何一步失败都停在该状态，记录具体原因后修复；不要跳到“已发布”，也不要因为“已上传”就声称商品已经公开。
...
登录、短信验证码、APP 扫码和最终“发布”确认必须由用户本人完成。除此之外，不应让用户重复提供已经存在于源目录、生成文件或分享块中的信息。
...
注意：`make_desc.py` 的 `write_project()` 会把 `quark_block` 拼进 `desc.txt` 和 `闲鱼发布文案_直接复制.txt`。因此该文件是“完整交付包”，不能整段粘贴到闲鱼正文。实际发布时复制商品正文部分（包含“拍后提供网盘链接”的虚拟资料说明），排除后面的 `文件夹名/链接/提取码` 分享块；分享块单独交付。
