...
每次发布都按下面的顺序执行，避免重复让用户补信息：

1. **准备素材**：生成并目视检查 `01.png` 至 `04.png`，确认图片数量、尺寸和文字布局。
2. **准备闲鱼文案**：正文只写项目内容、文件规模、适用人群和“拍后提供网盘链接”，不得出现 `pan.quark.cn`、分享 ID 或提取码。
3. **准备分享信息**：另外生成一段独立的交付文本，包含标题、私密永久分享链接和提取码。分享信息可以复制到剪贴板或单独发给买家，但不能粘贴进闲鱼正文。
4. **Playwright 上传**：将图片放入 Playwright 允许访问的临时上传目录，再通过文件选择器上传四张图片；不要把网盘链接填入表单。
5. **填写表单**：分类选择与虚拟资料匹配的可用分类，填写标题/正文、价格，并选择“无需邮寄”（如页面提供）。
6. **发布前复核**：确认正文没有 `pan.quark.cn`、分享 ID、提取码、价格、快递或物流承诺。
7. **发布确认**：最终点击“发布”前向用户展示标题、价格、图片数量、分类和正文检查结果；用户确认后再点击，并验证成功提示或商品链接。

标准交付文本模板（链接内容替换为本次项目实际值，不提交到 Git）：
...
- “已准备”：素材、文案和表单已就绪，尚未产生公开发布。
- “已上传”：图片已传入闲鱼页面，但尚未点击发布。
- “已发布”：点击后页面出现成功提示或商品详情链接。
