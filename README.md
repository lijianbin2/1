...
- **远程来源**：`cdn.jsdelivr.net/gh/powerfullz/override-rules/convert.min.js` 是 jsDelivr 的无版本入口，实际提供上游最新 Release 的构建产物；源码缓存有效期为 6 小时。上游 `main` 分支已迁移到 `src/*.ts`，不再直接跟踪 `convert.min.js`，因此不要把 GitHub `main/convert.min.js` 当作运行时入口。
- **内联回退**：`CONVERT_SNAPSHOT` 是仓库内置的历史快照，记录日期为 `2026-08-26`；它用于远程不可用时保持脚本可运行，不保证与当前 CDN 内容一致。
- **版本差异**：当前内联快照内容对应上游 `v2.5.5` 构建产物；截至 `2026-09-24` 审查时，jsDelivr 无版本入口返回的是更新的 `v2.7.6` 构建产物，二者 SHA-256 不一致。本项目是“运行时优先使用远程版本”，而不是“仓库内嵌始终是最新版”。
- **快照维护**：当前仓库没有自动构建快照的脚本；需要更新内联快照时，应从指定 Release 重新下载、嵌入并复核源码内容与日期。
...
| `loadbalance` | boolean | — | 兼容参数：`true` 会映射为 `grouptype=2`，`false` 允许继续使用 `grouptype`；建议新配置只传 `grouptype` |
...
if (Array.isArray(g.proxies) && g.proxies.includes("自动选择")) {
